...
let fitTextInElement = function (element) {
  ...
  console.log(`${element.id} -- default: ${defaultWidth} x ${defaultHeight}`);
  for (let testSize = 8; testSize <= 40; testSize += 0.5) {
    element.style.fontSize = `${testSize}px`;
    console.log(`testSize: ${testSize}, scroll: ${element.scrollWidth} x ${element.scrollHeight}`);
...
// Resize the text inside banner to fitt.
