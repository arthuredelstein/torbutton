...
// `"France (12.34.56.78)"`. Guard/bridge IP
// address and country remains hidden.
let nodeLines = function (nodeData) {
  let result = [], lineIndex = 0;
  for (let {ip, countryCode, type, bridgeType} of nodeData) {
    if (lineIndex === 0) {
      // For extra protection, don't show the IP address
      // or country for guard or bridge.
      if (type === "bridge") {
        result.push(
          uiString("tor_bridge") +
            ((bridgeType !== "vanilla") ? (": " + bridgeType) : ""));
      } else {
        result.push(uiString("tor_guard"));
      }
    } else {
      let country = countryCode ? localizedCountryNameFromCode(countryCode)
